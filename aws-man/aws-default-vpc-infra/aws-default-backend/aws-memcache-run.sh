#!/bin/bash
# The script to run a MemcacheD server on an Amazon Linux 2023 instance

source ./sandbox_env
SUBNET=$BACKEND_NAME
TARGET_IP=$MEMCACHE_IP
...
cd /tmp/provisioning
aws s3 cp s3://${BUCKET_NAME}/aws-vm/2-memcached.sh .
bash 2-memcached.sh"
...
if [ $? -eq 0 ]; then
    echo "An EC2 instance of MemcacheD server is running."
else
    echo "Something went wrong with MemcacheD server."
fi
